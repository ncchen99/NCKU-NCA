...
export default function FormResponsesPage() {
  ...
  const responseColumns = useMemo<ColumnDef<FormResponseRecord>[]>(() => {
    ...
    // Add up to 3 form field columns for the summary view
    ...
    for (const field of visibleFields) {
      cols.push({
        ...
        sortingFn: (rowA, rowB) =>
          compareZh(
            String(rowA.getValue(`field_${field.id}`)),
            String(rowB.getValue(`field_${field.id}`))
          ),
        cell: ({ row }) => {
          ...
          return (
            <span className="text-neutral-600 truncate max-w-[200px] inline-block">
              {display}
            </span>
          );
        },
    ...
    cols.push(
      ...
      },
      {
        ...
        enableSorting: false,
        cell: ({ row }) =>
          row.original.is_duplicate_attempt ? (
  ...
  // unique (non-duplicate) responses
  const uniqueResponses = useMemo(
    () => responses.filter((r) => !r.is_duplicate_attempt),
    [responses]
  );
